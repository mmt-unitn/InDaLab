...
categories: 
  - materiali
abstract: >
  In questa attività si conducono misure di durezza superficiale
---
...
E' importante distingure tra modelli statici (cioè che non tengono conto delle relazioni differenziali nel tempo delle grandezze del sistema) e modelli dinamici (che al contrario tengono conto dell'evoluzione nel tempo del sistema). Questa attività fa riferimento al primo.
...
flowchart TD
    ...
    title --> A(["Pianificazione della campagna di misure per t-test:<br>creazione della tabella di raccolta dati"])
    title --> B(["Pianificazione della campagna di misure per taratura:<br>creazione della tabella di raccolta dati"])
    
    A --> E([Raccolta delle misure in laboratorio])
    B --> E
    
    E --> F([Registrazione dei dati raccolti per t-test])
    E --> G([Registrazione dei dati raccolti per taratura])
```
...
anova(df_b.lm)%>%tidy()%>%kable()
```

Si verifica che i residui non abbiano pattern in funzione del Run Order e in funzione dei fattori 
```{r}
df_b %>% 
...
Va verificata l'assenza di pattern per i residui e la normalità degli stessi.
