...
df$stima_no = c(592, 588, 584, 217, 223, 222, 157, 611, 616, 263)
...
```{r}
df %>% ggplot(aes(x=stima_ok, y=stima_no)) + 
  geom_point() + 
  geom_smooth(method="lm", formula=y~x) +
  geom_abline(slope=1, intercept=0, color="red")
df %>% ggplot(aes(x=HV30_OK, y=HV30_NOK)) + 
  geom_point() + 
  geom_smooth(method="lm", formula=y~x) +
  geom_abline(slope=1, intercept=0, color="red")
```
...
```{r}
df %>% 
  ggplot(aes(x=HV30_OK, y=stima_ok)) +
  geom_point() +
  geom_smooth(method="lm", formula = y~x)
```
...
```{r}
t.test(df$HV30_OK,df$HV30_NOK,paired = T)
t.test(df$stima_ok,df$stima_no,paired = T)
```
{{< lipsum 2 >}}
