...
![Confronto tra scale di durezza diverse, Scienza ed ingegneria dei materiali, Callister & Rethwisch, *IV edizione*, p. 169](images/durezza/confronto_scale.jpg){width=22% fig-align="center"}
</div>
...
![Sistemi di prova per la durezza, Scienza ed ingegneria dei materiali, Callister & Rethwisch, <i>IV edizione</i>, p. 167](images/durezza/brinell_vickers.jpg){width=110%}
</div>
...
<div style="text-align:center">
![Campione prima e dopo preparazione superficiale](images/durezza/preparazione_campioni.jpg){#Preparazione_campioni width=15% .un}
</div>
...
<div style="text-align:center">
![Campioni numerati](images/durezza/campioni.jpg){width=15% #Campioni}
</div>
...
<div style="text-align:center">
![Provino a contatto](images/durezza/contatto.jpg){width=20% #provino}
</div>
...
<div style="text-align:center">
![Azzeramento del regolo](images/durezza/zeroregolo.jpg){width=15% #zeroregolo}
</div>
...
<div style="text-align:center">
![Orientamento regolo](images/durezza/orientamento.jpg){width=22% #orientamento}
</div>
...
<div style="text-align:center">
![Misura della diagonale](images/durezza/misura.jpg){width=18% #misura}
</div>
...
<div style="text-align:center">
![Durometro EMCO TEST M4U 02](images/durezza/durometro.jpg){width=12% #Durometro}
</div>
...
Si utilizza semopre la scala HV30.
...
Si osserva la relazione tra le misure fatte con indentatore rotto e indentatore integro tenendo presente che se non ci fosse differenza sarebbero allineate sulla retta diagonale con equazione $y=x$:
...
Dal grafico precedente si rileva che:
...
Questo significa che la stima dà una durezza maggiore per l'indentatore rotto che per quello integro.
...
Si confrontano le misure ottenute con doppia diagonale e quelle con vertice estrapolato da un indentatore danneggiato per stabilire quale sia più affidabile: se il p-value del confronto con la doppia diagonale è maggiore, allora conviene usare due volte la stessa diagonale.
...
E' necessario creare una tabella di raccolta dati dove per ogni materiale scelto (dall'1 al 6) si raccolgono i valori di stima e di durezza effettiva per ogni scala tra le 5 scelte (HV5, HV10, HV20, HV30, HV50). Si casualizza l'ordine di esecuzione delle prove.
...
kable(head(df_b,6), caption = "Tabella di raccolti dati per taratura")
...
Si regredisce una caratteristica lineare per ogni scala di durezza:
...
Si svolge l'analisi anvova considerando tutti i dati insieme per verificare che (come ci si aspetta) le regressioni dipendano sia dalla scala che dalla stima (ovviamente)

```{r}
df_b.lm <- lm(Hardness~Estimated + Scale, data = df_b)
...
df_b %>%
  group_by(Scale) %>%
  summarise(slope = lm(Hardness~Estimated - 1) %>% coefficients())
```
## Taratura 
...
df_b %>%
  split(~Scale) %>%
  map_dfr(~{
    d.lm <- lm(Hardness ~ Estimated - 1, data = .x)
    tibble(
...
```
## Taratura 
Va infine verificata l'assenza di pattern per i residui e la normalità degli stessi.
...
df_models <- df_b %>%
  ...
  map(~ {
    mod <- lm(Hardness ~ Estimated - 1, data = .x)
    .x %>%
      add_residuals(mod) %>%
...
# Residui vs Scale
ggplot(df_resid, aes(x = Scale, y = resid)) +
  geom_point() +
  labs(title = "Residui vs Scale")

# Residui vs Material
ggplot(df_resid, aes(x = Material, y = resid)) +
  geom_point() +
  labs(title = "Residui vs Material")

# Residui vs RunOrder
ggplot(df_resid, aes(x = RunOrder, y = resid)) +
  geom_point() +
  labs(title = "Residui vs RunOrder")

# QQ-plot per ogni scala
ggplot(df_resid, aes(sample = resid)) +
  geom_qq() +
  geom_qq_line() +
  facet_wrap(~ Scale, scales = "free") +
  labs(title = "QQ-plot dei residui per scala")

#Shapiro-Wilk  per ogni scala
df_shapiro <- df_resid %>%
  group_by(Scale) %>%
  summarise(
